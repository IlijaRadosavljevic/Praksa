...
@pytest.mark.parametrize(
    ...
    [
        ...
        ("titletitle", "contentcontent", False),
        ("title", "content", True),
    ],
)
...
def test_create_post_default_published_true(authorized_client, test_user, test_posts):
    res = authorized_client.post(
        "/posts/", json={"title": "probni title", "content": "probni content"}
    )

    created_post = schemas.Post(**res.json())

    assert res.status_code == 201
    assert created_post.title == "probni title"
    assert created_post.content == "probni content"
...
def test_unauthorized_user_update_posts(client, test_user, test_posts):
    res = client.put(f"/posts/json={test_posts[0].id}")
    assert res.status_code == 401
